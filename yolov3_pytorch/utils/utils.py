...
def model_info(model):
    ...
    n_g = sum(x.numel() for x in model.parameters() if x.requires_grad)  # number gradients （可训练的参数）
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    lxy, lwh, lcls, lconf = ft([0]), ft([0]), ft([0]), ft([0]) # ft([0]) → tensor([0.], device='cuda:0')
    ...
    bs = p[0].shape[0]  # batch size = 16
    k = h['k'] * bs  # loss gain ：10.39
...
###############################################
# 输入：targets.shape = (m,6)，其中m代表一个batch中Ground Truth的数量
# 处理：3 * m个anchorbox与Ground Truth进行IOU比较
#       取最大的IOU,得到m个比较结果
#       将m个IOU与iou阈值比较，得到n个结果，n <= m
# 返回：n个Ground Truth对应的txy,twh,tcls,indices
#       其中indices = (b, a, gj, gi)
#                      b表示这n个Ground Truth所在image的编号
#                      a表示与Ground Truth的IOU匹配最好的anchor
#                      gj表示n个Ground Truth的y坐标
#                      gj表示n个Ground Truth的x坐标
# 最后，由于是3个yolo层(也是3个Scale)，所以最终返回时上述结果的三组
##############################################
def build_targets(model, targets):
    ...
    for i in model.yolo_layers: # [82, 94, 106] for yolov3
        ...
        gwh = targets[:, 4:6] * layer.ng # layer.ng → (13,13) (26,26) (52,52)
                                         # ground truth wh
        '''
        print(layer.anchor_vec)
        tensor([[3.62500, 2.81250],  anchors/32  (13,13)
                [4.87500, 6.18750],
                [11.65625, 10.18750]], device='cuda:0')
        tensor([[1.87500, 3.81250],  anchors/16  (26,26)
                [3.87500, 2.81250],
                [3.68750, 7.43750]], device='cuda:0')
        tensor([[1.25000, 1.62500],  anchors/8   (52,52)
                [2.00000, 3.75000],
                [4.12500, 2.87500]], device='cuda:0')
        '''
        if nt:
            iou = [wh_iou(x, gwh) for x in layer.anchor_vec]
            iou, a = torch.stack(iou, 0).max(0)  # 与Ground Truth的IOU最大的anchor  返回最大IOU及对应索引
            # print(iou.shape) → torch.Size([75]) 75 表示一个batch中Ground Truth数量
            # reject below threshold ious (OPTIONAL, increases P, lowers R)
            reject = True
            if reject:
                j = iou > iou_thres #与Ground Truth的iou大于阈值的保留
                t, a, gwh = targets[j], a[j], gwh[j] #返回对应的targets anchro gwh

        # Indices
        b, c = t[:, :2].long().t()  # b ground truth  , class .t()  torch的转置
        gxy = t[:, 2:4] * layer.ng  # grid x, y
        gi, gj = gxy.long().t()  # grid x, y indices
        indices.append((b, a, gj, gi)) #grouth truth所在image编号  anchor  ground truth y x
